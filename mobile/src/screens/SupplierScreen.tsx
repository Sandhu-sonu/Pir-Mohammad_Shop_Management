...
export const SupplierScreen = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['suppliersList', page, search, status],
    queryFn: async () => {
        ...
        <Card.Content style={styles.cardContent}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: theme.colors.outline, fontSize: 12 }}>
              Mob: {item.mobile || 'N/A'} | GST: {item.gst || 'N/A'}
            </Text>
          </View>
          <View style={styles.balanceContainer}>
            <Text style={[styles.balance, { color: hasDues ? '#E67E22' : '#2ECC71' }]}>
              ₹{item.currentBalance}
            </Text>
    ...
    <View style={styles.container}>
      ...
      <SegmentedButtons
        ...
        buttons={[
          ...
          { value: 'outstanding', label: 'Dues Owed' },
        ]}
      ...
      {isLoading && page === 1 ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <Text style={styles.empty}>No supplier records found.</Text>
          }
          onEndReached={() => {
